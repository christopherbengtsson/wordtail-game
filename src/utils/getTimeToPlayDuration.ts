const BASE_TIME = 15 * 1000; // 15 sec
...
export function logarithmicDuration(numLetters: number) {
  ...
  return BASE_TIME + logMultiplier * Math.log(numLetters + 1);
}
...
export function quadraticDuration(numLetters: number) {
  ...
  return BASE_TIME + quadraticMultiplier * numLetters * numLetters;
}
...
export function sigmoidDuration(numLetters: number) {
  ...
  return BASE_TIME + maxExtraTime * (sigmoid(scale * numLetters) - sigmoid(0));
}
...
export function piecewiseDuration(numLetters: number) {
  if (numLetters <= 3) return BASE_TIME + numLetters * 1000;
  if (numLetters <= 7) return BASE_TIME + 3 * 1000 + (numLetters - 3) * 1500;
  return BASE_TIME + 3 * 1000 + 4 * 1500 + (numLetters - 7) * 2000;
}
...
export function randomDuration(numLetters: number) {
  ...
  return BASE_TIME + numLetters * timePerLetter + randomTime;
}
